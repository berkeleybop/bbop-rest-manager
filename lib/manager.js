...
function manager_base(response_handler){
	...
	if( typeof(url) !== 'undefined' ){ anchor.resource(url); }
	if( typeof(payload) !== 'undefined' ){ anchor.payload(payload); }
	if( typeof(method) !== 'undefined' ){ anchor.method(method); }

	// Since there is no AJAX/REST in our case, we just loop back
	// with the argument string.
	if( typeof(anchor.resource()) !== 'undefined' ){
